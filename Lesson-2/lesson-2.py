...
def parse_hh_page(url):
    ...
    vacations = soup.select("div.vacancy-serp div.vacancy-serp-item")
    data = []
    for vacation in vacations:
        data_item = {}

        data_item["site"] = "www.hh.ru"
        vacation_info = vacation.find(name="a", attrs={"data-qa": "vacancy-serp__vacancy-title"})
        data_item["name"] = vacation_info.text
        data_item["url"] = vacation_info["href"]
        # salary
        tag = vacation.find(name="span", attrs={"data-qa": "vacancy-serp__vacancy-compensation"})
        if tag is not None:
            # data_item["salary"] = tag.text.replace("\u202f", "")
            data_item["salary"] = tag.text.replace("\u202f", "")
        # employer
        tag = vacation.find(name="a", attrs={"data-qa": "vacancy-serp__vacancy-employer"})
        if tag is not None:
            data_item["employer"] = tag.text
        # location
        tag = vacation.find(name="span", attrs={"data-qa": "vacancy-serp__vacancy-address"})
        if tag is not None:
...
def parse_superjob_page(url):
    ...
    response = requests.get(url, headers=headers).text
    soup = BeautifulSoup(response, 'lxml')
    vacations = soup.select("div.f-test-search-result-item div.Fo44F")
    data = []
    for vacation in vacations:
        data_item = {}

        data_item["site"] = "www.superjob.ru"
        data_item["name"] = vacation.select("a._6AfZ9")[0].text
        data_item["url"] = "https://www.superjob.ru" + vacation.select("a.icMQ_")[0]["href"]
        # salary
        list = vacation.select("span.f-test-text-company-item-salary")
        if len(list) > 0:
            data_item["salary"] = list[0].text
        # employer
        list = vacation.select("a._205Zx")
        if len(list) > 0:
            data_item["employer"] = list[0].text
        # location
        list = vacation.select("span.f-test-text-company-item-location")
        if len(list) > 0:
...
d = request_hh_api("https://api.hh.ru/vacancies?text=Python&area=1&per_page=100")
pp.pprint(d)
print("\nTotal count: {}".format(len(d)))
...
#     print(". Found {} new vacations.".format(len(new_data)))
#     all_data += new_data
#
# print()
#
